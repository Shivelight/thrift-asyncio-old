...
string t_py_generator::py_imports() {
  ...
  // }
  // ss << "from thrift.Thrift import TType, TMessageType";
...
void t_py_generator::generate_py_struct_reader(ofstream& out, t_struct* tstruct) {
  ...
  if (is_immutable(tstruct)) {
    ...
    if (gen_asyncio_) {
      indent(out) << "async def read(self, iprot):" << endl;
    } else {
...
void t_py_generator::generate_service_client(t_service* tservice) {
  ...
  if (gen_twisted_) {
    ...
    f_service_ << "class Client(" << extends_client << "object):" << endl;
  }
  indent_up();
  generate_python_docstring(f_service_, tservice);
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    f_service_ << endl;
    if (gen_asyncio_) {
      indent(f_service_) << "async def send_" << function_signature(*f_iter, false) << ":" << endl;
    } else {
    ...
    if (!(*f_iter)->is_oneway()) {
      ...
      if (gen_twisted_ || gen_tornado_) {
        ...
        t_function recv_function((*f_iter)->get_returntype(),
                                 ...
                                 &noargs);
        if (gen_asyncio_) {
          f_service_ << indent() << "async def " << function_signature(&recv_function) << ":" << endl;
        } else {
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        ...
        if (gen_twisted_) {
          ...
          f_service_ << indent()
                     << "raise T";
          if (gen_asyncio_) {
            f_service_ << "AsyncioApplicationException(TAsyncioApplicationException.MISSING_RESULT, \""
                       << (*f_iter)->get_name() << " failed: unknown result\")" << endl;
          } else {
...
void t_py_generator::generate_service_server(t_service* tservice) {
  ...
  if (gen_asyncio_) {
    f_service_ << indent() << "async def process(self, iprot, oprot):" << endl;
  } else {
...
void t_py_generator::generate_process_function(t_service* tservice, t_function* tfunction) {
  ...
  if (gen_tornado_) {
    ...
    if (gen_asyncio_) {
      f_service_ << indent() << "async def process_" << tfunction->get_name()
...
THRIFT_REGISTER_GENERATOR(
    py,
    "Python",
    "    NOTE: Server and Remote services are not generated!\n"
